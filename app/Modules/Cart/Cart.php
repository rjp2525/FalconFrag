...
use Falcon\Modules\Cart\Exceptions\InvalidConditionException;
use Falcon\Modules\Cart\Exceptions\InvalidItemException;
use Falcon\Modules\Cart\Helpers\Helpers;
use Falcon\Modules\Cart\Validators\CartItemValidator;
...
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = '\\Falcon\\Modules\\Cart\\CartCondition';

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of Falcon\Modules\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition) {
            throw new InvalidConditionException('Argument 1 must be an instance of \'Falcon\Modules\Cart\CartCondition\'');
        }
